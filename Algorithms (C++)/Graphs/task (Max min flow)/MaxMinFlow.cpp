...
{
public:
	MaxMinFlow() : countSummit(0), countEdge(0), source(0), stok(0) {}
 
	friend std::istream& operator >> (std::istream& in, MaxMinFlow& a)
    {
        in >> a.countSummit >> a.countEdge;
        a.graph.resize(++a.countSummit);
	...
	void Solution()
	{
        source = 1; stok = countSummit - 1;
		std::pair<int, int> flow = MaxMinFlowDijkstra(source, stok, std::numeric_limits<int>::max());
        std::cout << flow << "\n";
	}
...
private:
	...
	int countSummit, countEdge, source, stok;
	...
	std::pair<int, int> MaxMinFlowDijkstra(const int& source, const int& stok, const int& maxFlow)
	{
