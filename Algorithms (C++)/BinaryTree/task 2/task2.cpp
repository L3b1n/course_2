...
public:
    ...
    bool Max(int& max);
    void Solution();
    int Hight(){ return Hight(pTree);}
    int Hight(KNodeTree* t);
...
int KBinaryTree::Hight(KNodeTree* t)
{
    if(t == nullptr){ return -1;}
    int leftHight = Hight(t->left);
    int rightHight = Hight(t->right);
    if(leftHight != rightHight){ numbers.push_back(t->info);}
    return std::max(leftHight, rightHight) + 1;
}

void KBinaryTree::Solution()
{
    Hight();
    if(numbers.size() % 2 != 0)
    {
