...
public class ZippedFileTester {
    ...
    public void TestGetResult() {
        try {
            FileBuilder builder = new FileBuilder("zip", "./src/main/tests/com/global_task/test.zip");
            builder.setZip(true);
    ...
    public void TestGetResultException() {
        try {
            FileBuilder builder = new FileBuilder("txt", "./src/main/results/com/global_task/test.zip");
            builder.setZip(true);
            InterfaceFileReader reader = builder.getFileReader();
            Assert.assertEquals("[[Result1 = 545.468], [Result2 = Error: devision by zero], [Result3 = 3.0]]", reader.Read().toString());
        } catch(Exception e) {
            Assert.assertEquals("Error in file unzip. Check selected file, actions and try again.", e.toString().substring(e.toString().indexOf(':') + 2));
        }
    }
}
